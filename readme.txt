myAut2Exe - The Open Source AutoIT Script Decompiler 2.15
=========================================================
...
=====
   ...
   'Function Renamer'
      ...
      A similar file can be a included 'include files' but can be also an older version
      of the script with intact names or some already recovered + manual improved with
      more meaningful function names.

      Bot files are shown side by side separated by their functions
      Here some example:
      ...
      ...                          |   $aMonthOfYear[1] = "January"
                                   ...
                                   |  ...
      Both function match with a double click or enter you can add them to the search'n'replace
...
=====
 ...
 data\AU3_BuildFuncs\3.3.14.2.dat.txt	used by DeTokeniser(to resolve tokenised Commands) & FuncRename (it skip these internal Autoit functions when exploring inner func calls)
 data\whxapi.dll	to automate Winhex
...
What are tokenized commands?
----------------------------

	Well normally tokens are coded like this:
	0x30 "GLOBAL"
	0x30 "CONST"
	...
	^- That's the way the official Aut2Exe compiler creates them.

	However I recently saw some script that used this from
		0x00 KEYWORDS[29]
		0x00 KEYWORDS[30]
	That depends on the internal array KEYWORDS[] in the script interpreter that looks like this:
		KEYWORDS[01]="AND"
		...
		KEYWORDS[29]="GLOBAL"
		KEYWORDS[30]="CONST"
	
	Thing about this that it saves store and loads fast.
	Don't know yet how to make the Aut2Exe compiler to use this. 
	
	CMD 0x00 are the keywords  like IF,FOR,GLOBAL... and has about 40 entries.
	CMD 0x01 are the internal AutoIt functions like Abs(),MsgBox()... and has about 400 entries.
	These I put into '3.3.14.2.dat.txt'.
...
That is the newer version called adaptive Huffman.
...
{1} 1Bit that is 1 and makes the algo to go into the else branch
...
Version 3_3
	...
	added new token for ternary operator
...
13.5.2017
	FunctionRenamer FunctionExplorer: Added matchpreview window
	FunctionRenamer: Delete an item is also removed from List_Fn_Assigned_Addedfuncs
	FunctionRenamer FunctionExplorer: on different varcount - double click removes varname

14.5.2017
	FunctionRenamer: validate Keywords
...
2.15 Detokeniser: Added support for tokenised commands (cmd opcodes 0x0 and 0x1)
	 ...
	 HexToBin: mod/special build for string deobfucation for the '7 Knights Script'
...
2.13 Added UPX unpacker
	 Added support for ternary operator [ Example: (a==4) ? "Okay" : "No"]
...
2.12 Reset button for OptionsDialog
     ...
     Updated tidy
     implemented 'separate include files' for AHK-File
...
2.10 Tools\GetAutoItVersion
     ...
     Minor bugfixes / isTextFile function; better error handling(very limited support for negative fileoffsets for the Filestreamclass)
     ...
     Created DeCompilerConfig.bas for better configuration
...
2.8 Added support for AHK 1.0.48.3 'N/A'-files
    ...
    separate includes of AHK-scripts
...
2.00 Improved command line handling + ne options /q /s
     options are saved
